## 目录
- [加载模型的方法](#加载模型的方法)
  - [通过命令行直接加载模型并交流](#通过命令行直接加载模型并交流)
  - [加载模型到本地同时开启一个本地服务器](#加载模型到本地同时开启一个本地服务器)
- [Safetensors转换为GGUF依靠llamacpp文件的方法](#Safetensors转换为GGUF依靠llamacpp文件的方法)
- [模型基于llamacpp内的文件进行量化](#模型基于llamacpp内的文件进行量化)
- [llamacpp-b9010-README文档翻译](#llamacpp-b9010-README文档翻译)

## 加载模型的方法（要求在llamacpp解压后的文件夹打开PowerShell，否则应该是找不到文件在哪）

### 通过命令行直接加载模型并交流
加载一个本地模型：` .\llama-cli -m "模型地址（要求写到.guff后缀为止）" -c 2048 -ngl 999 `。

#### 注：
- ` .\llama-cli `：启用名为llama-cli的程序，llamacpp解压文件中是有一个这个程序的。
- ` -m `：模型文件输入指令。
...
加载一个本地模型并设置一个OpenAI兼容的API服务：` .\llama-server -m "模型地址（要求写到.guff后缀为止）" --port "服务器端口" `。
...
## Safetensors转换为GGUF依靠llamacpp文件的方法
- 在llamacpp源文件夹中打开PowerShell；
- 安装依赖文件：` pip install -r requirements.txt `；
- 转换文件命令：` python convert_hf_to_gguf.py "Safetensors模型文件文件夹" --outfile "输出GGUF文件存放地址和名字（要求写到.guff后缀为止）" `。

### 注：
- 使用python3.13版会导致依赖文件安装错误，原因是文件中依赖库不支持这么新的版本（Deepseek说要用3.12版本以下才行），对于同一电脑不同版本python的调用，遵循在环境变量中，从上到下的顺序，优先使用较上的python文件。
- 如果不对` --outfile `安排存放地址只安排文件名，那么会将生成的文件直接放在llamacpp源文件夹中。

## 模型基于llamacpp内的文件进行量化
- 在llamacpp源文件夹中打开PowerShell；
- 运行量化文件：` .\llama-quantize.exe "模型地址（要求写到.guff后缀为止）" "量化模型输出地址（要求写到.guff后缀为止）"  量化类型 `

### 注：
- ` .\llama-quantize.exe `：启用名为llama-cli的程序，llamacpp解压文件中是有一个这个程序的。
- ` 量化类型 `：直接写入` Q4_K_M `即为Q4量化。
...
- **Hugging Face 缓存迁移：使用 `-hf` 下载的模型现在存储在标准 Hugging Face 缓存目录中，可以与其他 HF 工具共享。**
...
- [[反馈] 为支持下游消费者🤗提供更好的 llama.cpp 打包方式](https://github.com/ggml-org/llama.cpp/discussions/15313)
...
- 用于 FIM 代码补全的 VS Code 扩展：https://github.com/ggml-org/llama.vscode
- 用于 FIM 代码补全的 Vim/Neovim 插件：https://github.com/ggml-org/llama.vim
- Hugging Face 推理端点现在开箱即用支持 GGUF！https://github.com/ggml-org/llama.cpp/discussions/9669
...
入门 llama.cpp 非常简单。以下几种方法可以在您的机器上安装它：
...
- 克隆此仓库并从源码构建 - 查看 [构建指南](docs/build.md)

安装完成后，您需要准备一个模型。请转到 [获取和量化模型](#获取和量化模型) 部分了解更多信息。
...
`llama.cpp` 的主要目标是在各类硬件上实现 LLM 推理的最小化配置和顶尖性能——无论是在本地还是在云端。
...
- 1.5 位、2 位、3 位、4 位、5 位、6 位和 8 位整数量化，提高推理速度并减少内存使用
- 用于在 NVIDIA GPU 上运行 LLM 的自定义 CUDA 内核（通过 HIP 支持 AMD GPU，通过 MUSA 支持摩尔线程 GPU）
- Vulkan 和 SYCL 后端支持
- CPU+GPU 混合推理，可以部分加速总显存容量较大的模型

`llama.cpp` 项目是为 [ggml](https://github.com/ggml-org/ggml) 库开发新功能的主要实验场所。
...
- [X] [Vigogne (法文)](https://github.com/bofenghuang/vigogne)
...
- Rust (从 crates.io 自动化构建): [ShelbyJenkins/llm_client](https://github.com/ShelbyJenkins/llm_client)
...
- PHP (基于 llama.cpp 的 API 绑定及功能): [distantmagic/resonance](https://github.com/distantmagic/resonance) [(更多信息)](https://github.com/ggml-org/llama.cpp/pull/6326)
...
- [akx/ggify](https://github.com/akx/ggify) – 从 Hugging Face Hub 下载 PyTorch 模型并将其转换为 GGUF
- [akx/ollama-dl](https://github.com/akx/ollama-dl) – 从 Ollama 库下载模型，直接用于 llama.cpp
- [crashr/gppm](https://github.com/crashr/gppm) – 启动 llama.cpp 实例时降低使用 NVIDIA Tesla P40 或 P100 GPU 的空闲功耗
- [gpustack/gguf-parser](https://github.com/gpustack/gguf-parser-go/tree/main/cmd/gguf-parser) - 查看/检查 GGUF 文件并估算内存使用
- [Styled Lines](https://marketplace.unity.com/packages/tools/generative-ai/styled-lines-llama-cpp-model-292902) (专有许可，适用于 Unity3D 游戏开发的推理部分异步封装，包含预构建的移动和 Web 平台封装及示例模型)
- [unslothai/unsloth](https://github.com/unslothai/unsloth) – 🦥 将微调和训练后的模型导出/保存为 GGUF (Apache-2.0)
...
- [Paddler](https://github.com/intentee/paddler) - 开源 LLMOps 平台，可在您自己的基础设施中托管和扩展 AI
- [GPUStack](https://github.com/gpustack/gpustack) - 管理运行 LLM 的 GPU 集群
- [llama_cpp_canister](https://github.com/onicai/llama_cpp_canister) - 在互联网计算机上作为智能合约运行的 llama.cpp，使用 WebAssembly
- [llama-swap](https://github.com/mostlygeek/llama-swap) - 透明代理，为 llama-server 添加自动模型切换功能
- [Kalavai](https://github.com/kalavai-net/kalavai-client) - 在任何规模上众包端到端 LLM 部署
- [llmaz](https://github.com/InftyAI/llmaz) - ☸️ 用于 Kubernetes 上大型语言模型的简单、高级推理平台
- [LLMKube](https://github.com/defilantech/llmkube) - 适用于 llama.cpp 的 Kubernetes operator，支持多 GPU 和 Apple Silicon Metal
...
- [Lucy's Labyrinth](https://github.com/MorganRO8/Lucys_Labyrinth) - 一个简单的迷宫游戏，由 AI 模型控制的智能体会试图欺骗你。
...
| [WebGPU [进行中]](docs/build.md#webgpu) | 全部 |
...
[Hugging Face](https://huggingface.co) 平台托管了众多与 `llama.cpp` 兼容的 [大语言模型](https://huggingface.co/models?library=gguf&sort=trending)：

- [热门模型](https://huggingface.co/models?library=gguf&sort=trending)
- [LLaMA](https://huggingface.co/models?sort=trending&search=llama+gguf)

您可以手动下载 GGUF 文件，或者通过此 CLI 参数 `-hf <user>/<model>[:quant]` 直接从 [Hugging Face](https://huggingface.co/) 或其他模型托管站点使用任何兼容 `llama.cpp` 的模型。例如：

```sh
llama-cli -hf ggml-org/gemma-3-1b-it-GGUF
```

默认情况下，CLI 会从 Hugging Face 下载，您可以通过环境变量 `MODEL_ENDPOINT` 切换至其他选项。`MODEL_ENDPOINT` 必须指向兼容 Hugging Face 的 API 端点。

下载模型后，请使用 CLI 工具在本地运行它——见下文。
...
Hugging Face 平台提供了多种在线工具，用于转换、量化和托管 `llama.cpp` 的模型：

- 使用 [GGUF-my-repo space](https://huggingface.co/spaces/ggml-org/gguf-my-repo) 将模型转换为 GGUF 格式并量化权重到更小的尺寸
...
#### 一个用于访问和实验 `llama.cpp` 大部分功能的命令行工具。

- <details open>
    <summary>以对话模式运行</summary>

    内置聊天模板的模型会自动激活对话模式。如果没有，您可以手动启用，方法是添加 `-cnv` 并使用 `--chat-template NAME` 指定合适的聊天模板。
...
- <details>
    ...
    对于编写更复杂的 JSON 语法，请参阅 https://grammar.intrinsiclabs.ai/
...
#### 一个轻量级、[兼容 OpenAI API](https://github.com/openai/openai-openapi) 的 HTTP 服务器，用于提供 LLM 服务。

- <details open>
    <summary>使用默认配置在端口 8080 上启动本地 HTTP 服务器</summary>

    ```bash
    llama-server -m model.gguf --port 8080

    # 基本 Web 界面可通过浏览器访问：http://localhost:8080
...
- <details>
    <summary>启用推测性解码</summary>

    ```bash
    # draft.gguf 模型应是目标 model.gguf 的一个较小变体
...
- 贡献者可以发起 Pull Request
- 协作者将基于贡献被邀请加入
- 维护者可以推送至 `llama.cpp` 仓库的分支，并合并 PR 至 `master` 分支
- 欢迎任何帮助管理 issue、PR 和项目的行为！
- 请参阅 [good first issues](https://github.com/ggml-org/llama.cpp/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) 寻找适合首次贡献的任务
- 阅读 [CONTRIBUTING.md](CONTRIBUTING.md) 了解更多信息
- 请务必阅读此文：[边缘上的推理](https://github.com/ggml-org/llama.cpp/discussions/205)
- 感兴趣者的一些背景故事：[Changelog 播客](https://changelog.com/podcast/532)
...
如果您的问题与模型生成质量有关，请至少浏览以下链接和论文，以理解 LLaMA 模型的局限性。这对于选择适当的模型规模以及认识到 LLaMA 模型与 ChatGPT 之间的显著细微差异尤其重要：
- LLaMA:
...
## XCFramework
XCFramework 是库的预编译版本，适用于 iOS、visionOS、tvOS 和 macOS。可在 Swift 项目中使用，无需从源码编译。例如：
```swift
// swift-tools-version: 5.10
// swift-tools-version 声明了构建此包所需的最低 Swift 版本。
...
```
上面示例使用的是库的中间版本 `b5046`。可以更改为使用不同版本，只需修改 URL 和校验和。
...
```
可以选择将其添加到您的 `.bashrc` 或 `.bash_profile` 中以自动加载。例如：
```console
...
- [yhirose/cpp-httplib](https://github.com/yhirose/cpp-httplib) - 单头文件 HTTP 服务器，用于 `llama-server` - MIT 许可证
- [stb-image](https://github.com/nothings/stb) - 单头文件图像格式解码器，用于多模态子系统 - 公共领域
- [nlohmann/json](https://github.com/nlohmann/json) - 单头文件 JSON 库，用于多种工具/示例 - MIT 许可证
- [miniaudio.h](https://github.com/mackron/miniaudio) - 单头文件音频格式解码器，用于多模态子系统 - 公共领域
